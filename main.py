    ...
    def stream_generator():
        ...
        while True:
            ...
            if current_frame is not None:
                ...
                if (current_app == 'dist_detect' or current_app == 'object_detect') and streaming_queues.c_queue.empty(): 
                    if globalvar.click_screenshot :
                        result_img = globalvar.screen_shot_picture
                    else:
                        result_img = vf_main.get_img(current_frame)
...
if __name__ == '__main__':
    ...
    # 物件偵測 + 虛擬圍籬 初始化
    from vgpy.vf.vf_api import  VfVar, VFMain
    vfvar = VfVar()    
    vf_main = VFMain(streaming_queues, vfvar, globalvar)
    
    app_vf = vf_main.vf_app_init()
    app_obj = vf_main.obj_app_init()
    app.register_blueprint(app_vf)
    app.register_blueprint(app_obj)
